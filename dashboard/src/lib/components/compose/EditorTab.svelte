<script lang="ts">
	...
	let yamlText = $state('');
	let issues = $state<{ field: string; message: string; severity: string }[]>([]);
	...
	async function runValidation() {
		...
		validating = true;
		try {
			const result = await validateCompose(yamlText);
			issues = result.issues;
			// Check for name change
			const match = yamlText.match(/^\s*name:\s*(.+)$/m);
			const parsedName = match?.[1]?.trim();
			nameWarning = !!parsedName && parsedName !== composeName;
		} catch {
	...
	async function handleSave() {
		...
		saveSuccess = false;
		try {
			await saveComposeYaml(composeId, yamlText);
			saveSuccess = true;
			onSaved?.();
		} catch (err) {
	...
	{#if nameWarning}
		<div class="flex items-start gap-2 border-l-2 border-l-warn bg-warn/5 px-3 py-2">
			...
			<p class="text-[13px] text-fg-muted">
				Changing the compose name will split event history. Existing events will remain under the previous name.
			</p>
		</div>
	{/if}
