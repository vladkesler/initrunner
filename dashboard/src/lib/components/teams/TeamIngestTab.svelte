<script lang="ts">
	...
	import {
		getTeamIngestDocuments,
		getTeamIngestSummary,
		deleteTeamIngestDocument,
		addTeamIngestUrl,
	...
	async function loadData() {
		...
		try {
			const [docs, sum] = await Promise.all([
				getTeamIngestDocuments(teamId),
				getTeamIngestSummary(teamId)
			]);
			documents = docs;
			summary = sum;
		} catch {
	...
	function handleIngest(force: boolean) {
		ingesting = true;
		ingestProgress = [];
		ingestController = streamTeamIngest(teamId, force, {
	...
	async function handleUpload(event: Event) {
		...
		uploading = true;
		try {
			await uploadTeamIngestFiles(teamId, input.files);
			await loadData();
		} catch {
	...
	async function handleAddUrl() {
		...
		addingUrl = true;
		try {
			await addTeamIngestUrl(teamId, url);
			urlValue = '';
			showUrlInput = false;
			await loadData();
		} catch {
	...
	function handleDelete(source: string) {
		if (confirmDeleteSource === source) {
			...
			deleteTeamIngestDocument(teamId, source).catch(() => { toast.error('Failed to delete document'); loadData(); });
		} else {
			if (confirmTimer) clearTimeout(confirmTimer);
...
{#if !hasIngest}
	<div class="flex flex-col items-center justify-center py-16 text-center">
		<p class="text-[13px] text-fg-faint">This team has no shared documents configuration.</p>
		<p class="mt-1 text-[12px] text-fg-faint">
			Add a shared_documents block with enabled: true to the team YAML to enable document management.
		</p>
	</div>
{:else}
	<div class="space-y-4">
		...
		{:else if documents.length === 0}
			<div class="flex flex-col items-center justify-center py-16 text-center">
				...
				<p class="mt-1 text-[12px] text-fg-faint">
					Upload files or add URLs to populate the shared document store.
				</p>
			</div>
		{:else}
			<div class="space-y-2">
