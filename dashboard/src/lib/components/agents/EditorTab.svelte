<script lang="ts">
	...
	import type { ValidationIssue } from '$lib/api/builder';
	...
	async function runValidation() {
		...
		try {
			...
			const match = yamlText.match(/^name:\s*(.+)$/m);
			const parsedName = match?.[1]?.trim();
			nameWarning = !!parsedName && parsedName !== agentName;
		} catch {
	...
	{#if nameWarning}
		<div class="flex items-start gap-2 border-l-2 border-l-warn bg-warn/5 px-3 py-2">
			...
			<p class="text-[13px] text-fg-muted">
				Changing the agent name will disconnect run history and memory from the previous name.
			</p>
		</div>
	{/if}
