"""Zvec-backed MemoryStore implementation."""
...
class ZvecMemoryStore(MemoryStoreBase):
    ...
    def meta_col(self) -> zvec.Collection:
        ...
        if self._meta_col is None:
            raise RuntimeError("ZvecMemoryStore is closed (meta collection released)")
        return self._meta_col

    @property
    def sessions_col(self) -> zvec.Collection:
        """Return sessions collection, raising if closed."""
        if self._sessions_col is None:
            raise RuntimeError("ZvecMemoryStore is closed (sessions collection released)")
        return self._sessions_col

    @property
    def memories_col(self) -> zvec.Collection:
        """Return memories collection, raising if not initialized."""
        if self._memories_col is None:
            raise RuntimeError("Memories collection not initialized (dimensions unknown)")
        return self._memories_col
    ...
    def _ensure_vec_collection(self, dimensions: int) -> None:
        ...
        if self._dimensions is not None:
            return
        _write_meta(self.meta_col, "dimensions", str(dimensions))
    ...
    def _flush_counters(self) -> None:
        meta = self.meta_col
        _write_meta(meta, "next_session_id", str(self._next_session_id))
        _write_meta(meta, "next_memory_id", str(self._next_memory_id))
    ...
    def load_latest_session(self, agent_name: str, max_messages: int = 20) -> list | None:
        with self._lock:
            results = self.sessions_col.query(
    ...
    def delete_session(self, session_id: str, agent_name: str) -> bool:
        with self._lock:
            ...
            if not results:
                return False
            sessions.delete(ids=[doc.id for doc in results])
            return True
