...
- prompt is the system prompt. If one was extracted, use it as-is. If none was found, \
...
- Only set reasoning.pattern: react when the summary says agent_kind: react.
...
- Do NOT add memory. LangChain memory does not map cleanly to InitRunner memory.
- If temperature or max_tokens were extracted, include them in model.
- If max_iterations was extracted, set guardrails.max_iterations.
- If structured output was detected, set output.type: json_schema and describe \
