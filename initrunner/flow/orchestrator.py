...
Uses pydantic-graph beta for parallel execution of agent delegation
chains.  Each flow agent becomes a graph step; fan-out becomes
...
class AgentStepResult:
    """Per-agent result from a ``run_once`` execution."""
...
class FlowMember:
    """Wraps a single agent within a flow orchestration.
...
class FlowOrchestrator:
    """Manages flow agent lifecycle and graph execution.
    ...
    @property
    def members(self) -> dict[str, FlowMember]:
    ...
    def _build_members(self, *, one_shot: bool = False) -> None:
        """Load roles and create FlowMember instances.
        ...
        or file-write sinks.
        """
        shared_mem = self._flow.spec.shared_memory
        shared_doc = self._flow.spec.shared_documents
        ...
        if shared_mem.enabled:
            ...
            shared_mem_path = shared_mem.store_path or str(
                DEFAULT_MEMORY_DIR / f"{self._flow.metadata.name}-shared.db"
            )
        ...
        if shared_doc.enabled:
            ...
            shared_doc_path = shared_doc.store_path or str(
                DEFAULT_STORES_DIR / f"{self._flow.metadata.name}-shared.lance"
            )
            ...
            try:
                ...
                if should_build_role_sinks and role.spec.sinks:
                    ...
                    for sink_config in role.spec.sinks:
                        ...
                        if sink:
                            member.add_sink(sink)

                self._members[name] = member
            except Exception as exc:
                error_msg = f"{type(exc).__name__}: {exc}"
                self._failed_members[name] = error_msg
                console.print(f"[red]Failed to build agent '{name}': {error_msg}[/red]")

        if not self._members:
            failed = ", ".join(self._failed_members.keys())
            raise RuntimeError(f"All agents failed to build: {failed}")
    ...
    def _topological_order(self) -> list[list[str]]:
        """Return agents in topological tiers based on needs."""
        from initrunner._graph import topological_tiers

        nodes = set(self._members.keys())
        edges = {
            name: [dep for dep in config.needs if dep in nodes]
            for name, config in self._flow.spec.agents.items()
            if name in nodes
        }
        return topological_tiers(nodes, edges)

    def _find_entry(self, entry_service: str | None = None) -> FlowMember:
        """Identify the entry agent (first with no incoming sink targets)."""
        if entry_service is not None:
            if entry_service not in self._members:
                raise ValueError(f"Entry agent '{entry_service}' not found")
            return self._members[entry_service]

        targeted: set[str] = set()
        for config in self._flow.spec.agents.values():
        ...
        for tier in self._topological_order():
            for name in tier:
                if name not in targeted and name in self._members:
    ...
    def start(self) -> None:
        """Build agents and start daemon in a background thread."""
        from initrunner.flow.graph import start_daemon
    ...
    def agent_health(self) -> list[dict]:
        """Return per-agent run/error stats."""
        return [
            {
                "agent": name,
                "runs": svc.run_count,
            ...
            for name, svc in self._members.items()
        ]
    ...
    def _collect_results(
        ...
        *,
        ...
        """Collect per-agent results into a FlowRunResult."""
        ...
        terminal_steps = [s for s in steps if s.agent_name in terminal_names]
        if len(terminal_steps) == 1:
            output_mode = "single"
            final_name = terminal_steps[0].agent_name
    ...
    def _log_aggregate(
        ...
        """Log a top-level aggregate audit row for the flow run."""
        ...
        self._audit_logger.log(
            AuditRecord(
                run_id=flow_run_id,
                agent_name=self._flow.metadata.name,
                ...
                trigger_metadata=json.dumps(
                    {
                        "flow_name": self._flow.metadata.name,
                        "flow_run_id": flow_run_id,
                        "scope": "aggregate",
                        "final_agent_name": result.final_agent_name,
    ...
    def __enter__(self) -> FlowOrchestrator:
        self.start()
        return self
...
def _print_shutdown_summary(orchestrator: FlowOrchestrator) -> None:
    """Print a summary table after flow shutdown."""
    svc_table = Table(title="Agent Summary")
    svc_table.add_column("Agent", style="cyan")
    svc_table.add_column("Status")
    svc_table.add_column("Runs", justify="right")
    svc_table.add_column("Errors", justify="right")

    for name, svc in orchestrator.members.items():
    ...
    for name, reason in orchestrator._failed_members.items():
        svc_table.add_row(name, "[red]failed[/red]", "-", reason)

    console.print(svc_table)

    health = orchestrator.agent_health()
    if health:
...
def run_flow(
    ...
    *,
    ...
    for name, config in flow.spec.agents.items():
        ...
        deps_str = ", ".join(config.needs) if config.needs else "(none)"
    ...
    with orchestrator:
        ...
        def _on_first_signal() -> None:
            console.print("\n[yellow]Shutting down flow...[/yellow]")
