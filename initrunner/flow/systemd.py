"""Systemd user unit integration for flow orchestration."""
...
[Unit]
Description=InitRunner Flow: {flow_name}
...
_ENV_TEMPLATE = """\
# Environment file for initrunner flow: {flow_name}
...
def sanitize_unit_name(flow_name: str, flow_path: Path | None = None) -> str:
    """Convert a flow name to a safe systemd unit name component.
    ...
    If *flow_path* is given and a unit already exists at the target path
    with a different WorkingDirectory, appends a 4-char hash to disambiguate.
    """
    sanitized = re.sub(r"[^a-zA-Z0-9_-]", "-", flow_name)
    ...
    if flow_path is not None:
        ...
        if existing_unit.exists():
            ...
            work_dir = str(flow_path.resolve().parent)
            if f"WorkingDirectory={work_dir}" not in content:
                path_hash = hashlib.sha256(str(flow_path.resolve()).encode()).hexdigest()[:4]
                sanitized = f"{sanitized}-{path_hash}"

    return sanitized


def unit_name_for(flow_name: str) -> str:
    """Return the full systemd unit name for a flow orchestration."""
    return f"initrunner-{sanitize_unit_name(flow_name)}.service"
...
def generate_unit_content(
    ...
    *,
    ...
    """Render a systemd unit file for the given flow orchestration."""
    flow_path = flow_path.resolve()
    sanitized = sanitize_unit_name(flow_name, flow_path)
    ...
    if " -m " in exe:
        ...
        exec_start = (
            ...
            f"flow up {_systemd_escape_arg(str(flow_path))}"
        )
    else:
        exec_start = f"{_systemd_escape_arg(exe)} flow up {_systemd_escape_arg(str(flow_path))}"
...
def generate_env_template(flow_name: str) -> str:
    """Return placeholder .env content for a flow orchestration."""
    return _ENV_TEMPLATE.format(flow_name=flow_name)
...
def install_unit(
    ...
    *,
    ...
    content = generate_unit_content(flow_name, flow_path, executable=executable, env_file=env_file)
...
def uninstall_unit(flow_name: str) -> Path:
...
def get_unit_status(flow_name: str) -> str:
    """Return the ``systemctl status`` output for a flow unit."""
    check_systemd_available()
    sanitized = sanitize_unit_name(flow_name)
    unit_name = f"initrunner-{sanitized}.service"
...
def resolve_flow_name(name_or_path: str) -> str:
    """Resolve a flow name from a name string or YAML path.

    If the argument looks like a path (has a YAML extension or exists on disk),
    load the flow definition and return ``metadata.name``.
    If it has a YAML extension but does not exist, raise ``FileNotFoundError``.
    Otherwise, return the argument as-is (assumed to be a flow name).
    """
    p = Path(name_or_path)
    ...
    if is_yaml or p.exists():
        if not p.exists():
            raise FileNotFoundError(f"Flow file not found: {name_or_path}")
