...
YAML ファーストの AI エージェントプラットフォーム。エージェントのロール、ツール、ナレッジベース、メモリを 1 つのファイルで定義。インタラクティブチャット、ワンショットコマンド、cron/webhook/ファイル監視トリガー付き自律デーモン、Telegram/Discord ボット、OpenAI 互換 API として実行可能。RAG と永続メモリはすぐに使えます。Web ダッシュボードまたはネイティブデスクトップアプリですべてを管理。`curl` または `pip` でインストール、コンテナ不要。
...
> **v2026.4.2**: PydanticAI + LangChain エージェントインポート。`initrunner new --pydantic-ai my_agent.py` または `--langchain` で既存エージェントを変換。[変更履歴](CHANGELOG.md) を参照。
...
同じファイルをインタラクティブチャット（`-i`）、ワンショットコマンド（`-p "..."`）、cron/webhook/ファイル監視デーモン（`--daemon`）、または OpenAI 互換 API（`--serve`）として実行できます。デプロイモードを事前に選んでそれに合わせて構築する必要はありません。実行時にフラグで選ぶだけです。
