...
from initrunner.stores.lance_store import (
    LanceDocumentStore,
    LanceMemoryStore,
    wipe_document_store,
)
...
class TestDocumentStoreDimensions:
    def test_auto_detect_from_existing_db(self, tmp_path):
        ...
        with LanceDocumentStore(store_path, dimensions=768) as store:
            assert store.dimensions == 768

        with LanceDocumentStore(store_path) as store:
            assert store.dimensions == 768

    def test_dimension_mismatch_raises(self, tmp_path):
        """Create store with 768d, reopen with 1536d, verify error."""
        store_path = tmp_path / "test.lance"
        with LanceDocumentStore(store_path, dimensions=768):
            pass

        with pytest.raises(DimensionMismatchError, match=r"768d.*1536d"):
            LanceDocumentStore(store_path, dimensions=1536)
...
class TestMemoryStoreDimensions:
    def test_auto_detect_from_existing_db(self, tmp_path):
        ...
        store_path = tmp_path / "test.lance"
        with LanceMemoryStore(store_path, dimensions=768) as store:
            assert store.dimensions == 768
    ...
    def test_dimension_mismatch_raises(self, tmp_path):
        """Create store with 768d, reopen with 1536d, verify error."""
        store_path = tmp_path / "test.lance"
        with LanceMemoryStore(store_path, dimensions=768):
            pass

        with pytest.raises(DimensionMismatchError, match=r"768d.*1536d"):
            LanceMemoryStore(store_path, dimensions=1536)
