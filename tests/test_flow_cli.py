...
class TestFlowValidate:
    def test_valid_flow(self, tmp_path):
        flow_file = _write_flow(tmp_path)
        _write_roles(tmp_path)

        result = runner.invoke(app, ["flow", "validate", str(flow_file)])
        assert result.exit_code == 0
        assert "Valid" in result.output

    def test_missing_flow_file(self, tmp_path):
        result = runner.invoke(app, ["flow", "validate", str(tmp_path / "nope.yaml")])
        assert result.exit_code == 1

    def test_invalid_flow(self, tmp_path):
        f = tmp_path / "bad.yaml"
        f.write_text("apiVersion: initrunner/v1\nkind: Flow\n")
        result = runner.invoke(app, ["flow", "validate", str(f)])
        assert result.exit_code == 1
    ...
    def test_displays_agent_table(self, tmp_path):
        flow_file = _write_flow(tmp_path)
        _write_roles(tmp_path)
...
class TestFlowUp:
    @patch("initrunner.cli.flow_cmd.create_audit_logger")
    @patch("initrunner.flow.orchestrator.run_flow")
    @patch("initrunner.flow.loader.load_flow")
    def test_up_calls_run_flow(self, mock_load, mock_run, mock_audit, tmp_path):
        ...
        runner.invoke(app, ["flow", "up", str(flow_file)])
        mock_run.assert_called_once()

    def test_up_missing_file(self, tmp_path):
        result = runner.invoke(app, ["flow", "up", str(tmp_path / "nope.yaml")])
        assert result.exit_code == 1

    def test_up_invalid_flow(self, tmp_path):
        f = tmp_path / "bad.yaml"
        f.write_text("not: valid\n")
        result = runner.invoke(app, ["flow", "up", str(f)])
        assert result.exit_code == 1
...
class TestFlowInstall:
    @patch("initrunner.flow.systemd.check_linger_enabled", return_value=True)
    @patch("initrunner.flow.systemd.install_unit")
    def test_valid_flow(self, mock_install, mock_linger, tmp_path):
        ...
        result = runner.invoke(app, ["flow", "install", str(flow_file)])
        assert result.exit_code == 0
        assert "Installed" in result.output
        assert "initrunner-test-flow.service" in result.output
        mock_install.assert_called_once()

    def test_invalid_flow(self, tmp_path):
        f = tmp_path / "bad.yaml"
        f.write_text("not: valid\n")
        result = runner.invoke(app, ["flow", "install", str(f)])
        assert result.exit_code == 1

    def test_missing_flow_file(self, tmp_path):
        result = runner.invoke(app, ["flow", "install", str(tmp_path / "nope.yaml")])
        assert result.exit_code == 1
    ...
    def test_force_overwrite(self, mock_install, mock_linger, tmp_path):
        ...
        result = runner.invoke(app, ["flow", "install", "--force", str(flow_file)])
        assert result.exit_code == 0
        call_kwargs = mock_install.call_args
        assert call_kwargs[1]["force"] is True

    @patch("initrunner.flow.systemd.install_unit")
    @patch("initrunner.flow.systemd.check_linger_enabled", return_value=False)
    ...
    def test_generate_env(self, mock_install, mock_linger, tmp_path):
        ...
        result = runner.invoke(app, ["flow", "install", "--generate-env", str(flow_file)])
        assert result.exit_code == 0
...
class TestFlowUninstall:
    @patch("initrunner.flow.systemd.uninstall_unit")
    def test_by_name(self, mock_uninstall):
        ...
        result = runner.invoke(app, ["flow", "uninstall", "proj"])
        assert result.exit_code == 0
        assert "Uninstalled" in result.output

    @patch("initrunner.flow.systemd.uninstall_unit")
    @patch("initrunner.flow.systemd.resolve_flow_name", return_value="test-flow")
    def test_by_path(self, mock_resolve, mock_uninstall, tmp_path):
        flow_file = _write_flow(tmp_path)
        mock_uninstall.return_value = Path(
            "/home/user/.config/systemd/user/initrunner-test-flow.service"
        )
        result = runner.invoke(app, ["flow", "uninstall", str(flow_file)])
        assert result.exit_code == 0
        mock_resolve.assert_called_once_with(str(flow_file))


class TestFlowStart:
    @patch("subprocess.run")
    @patch("initrunner.flow.systemd.check_systemd_available")
    def test_calls_systemctl(self, mock_check, mock_run, tmp_path):
        mock_run.return_value = MagicMock(returncode=0)
        result = runner.invoke(app, ["flow", "start", "my-proj"])
        assert result.exit_code == 0
        mock_run.assert_called_once()
...
class TestFlowStop:
    ...
    def test_calls_systemctl(self, mock_check, mock_run):
        mock_run.return_value = MagicMock(returncode=0)
        result = runner.invoke(app, ["flow", "stop", "my-proj"])
        assert result.exit_code == 0
        args = mock_run.call_args[0][0]
        assert args == ["systemctl", "--user", "stop", "initrunner-my-proj.service"]


class TestFlowRestart:
    @patch("subprocess.run")
    @patch("initrunner.flow.systemd.check_systemd_available")
    def test_calls_systemctl(self, mock_check, mock_run):
        mock_run.return_value = MagicMock(returncode=0)
        result = runner.invoke(app, ["flow", "restart", "my-proj"])
        assert result.exit_code == 0
        args = mock_run.call_args[0][0]
...
class TestFlowStatus:
    @patch("initrunner.flow.systemd.get_unit_status", return_value="active (running)")
    def test_output_forwarded(self, mock_status):
        result = runner.invoke(app, ["flow", "status", "my-proj"])
        assert result.exit_code == 0
        assert "active (running)" in result.output

    @patch(
        "initrunner.flow.systemd.get_unit_status",
        side_effect=lambda n: (_ for _ in ()).throw(
            __import__("initrunner.flow.systemd", fromlist=["SystemdError"]).SystemdError(
                "not found"
            )
        ),
    )
    def test_error_handling(self, mock_status):
        from initrunner.flow.systemd import SystemdError

        with patch("initrunner.flow.systemd.get_unit_status", side_effect=SystemdError("fail")):
            result = runner.invoke(app, ["flow", "status", "my-proj"])
        assert result.exit_code == 1
...
class TestFlowLogs:
    ...
    def test_journalctl_args(self, mock_check, mock_run):
        mock_run.return_value = MagicMock(returncode=0)
        result = runner.invoke(app, ["flow", "logs", "my-proj"])
        assert result.exit_code == 0
        args = mock_run.call_args[0][0]
    ...
    def test_follow_flag(self, mock_check, mock_run):
        mock_run.return_value = MagicMock(returncode=0)
        result = runner.invoke(app, ["flow", "logs", "--follow", "my-proj"])
        assert result.exit_code == 0
        args = mock_run.call_args[0][0]
    ...
    def test_custom_lines(self, mock_check, mock_run):
        mock_run.return_value = MagicMock(returncode=0)
        result = runner.invoke(app, ["flow", "logs", "--lines", "100", "my-proj"])
        assert result.exit_code == 0
        args = mock_run.call_args[0][0]
...
class TestFlowEvents:
    def test_no_db_exits_with_error(self, tmp_path):
        result = runner.invoke(
            app,
            ["flow", "events", "--audit-db", str(tmp_path / "nonexistent.db")],
        )
        assert result.exit_code == 1
        assert "not found" in result.output
    ...
    def test_empty_db_shows_message(self, tmp_path):
        ...
        result = runner.invoke(
            app,
            ["flow", "events", "--audit-db", str(db_path)],
        )
        assert result.exit_code == 0
    ...
    def test_status_filter(self, tmp_path):
        ...
        result = runner.invoke(
            app,
            ["flow", "events", "--status", "filtered", "--audit-db", str(db_path)],
        )
        assert result.exit_code == 0
    ...
    def test_run_id_filter(self, tmp_path):
        ...
        result = runner.invoke(
            app,
            ["flow", "events", "--run-id", "target-run", "--audit-db", str(db_path)],
        )
        assert result.exit_code == 0
