"""Tests for systemd integration (flow/systemd.py)."""
...
class TestSanitizeUnitName:
    ...
    def test_disambiguates_with_hash(self, tmp_path):
        ...
        flow_path = tmp_path / "sub" / "flow.yaml"
        flow_path.parent.mkdir(parents=True)
        flow_path.write_text("")

        with patch("initrunner.flow.systemd._UNIT_DIR", unit_dir):
            result = sanitize_unit_name("myproj", flow_path)
...
class TestGenerateUnitContent:
    def test_contains_exec_start(self, tmp_path):
        ...
        content = generate_unit_content("test-proj", flow_path, executable="/usr/bin/initrunner")
        assert f"ExecStart=/usr/bin/initrunner flow up {flow_path.resolve()}" in content

    def test_exec_start_quotes_path_with_spaces(self, tmp_path):
        spaced = tmp_path / "my project"
        spaced.mkdir()
        flow_path = spaced / "flow.yaml"
        flow_path.write_text("")

        content = generate_unit_content("test-proj", flow_path, executable="/usr/bin/initrunner")
        resolved = str(flow_path.resolve())
        assert f'ExecStart=/usr/bin/initrunner flow up "{resolved}"' in content
    ...
    def test_read_write_paths_quotes_spaces(self, tmp_path):
        ...
        content = generate_unit_content("test-proj", flow_path, executable="/usr/bin/initrunner")
        resolved_dir = str(flow_path.resolve().parent)
    ...
    def test_contains_working_directory(self, tmp_path):
        ...
        content = generate_unit_content("test-proj", flow_path)
    ...
    def test_custom_env_file(self, tmp_path):
        ...
        content = generate_unit_content("test-proj", flow_path, env_file=env_file)
...
class TestInstallUnit:
    @patch("initrunner.flow.systemd._systemctl")
    ...
    def test_creates_unit_file(self, mock_check, mock_ctl, tmp_path):
        ...
        with patch("initrunner.flow.systemd._UNIT_DIR", unit_dir):
            info = install_unit("test-proj", flow_path, executable="/usr/bin/initrunner")
    ...
    def test_daemon_reload_called(self, mock_check, mock_ctl, tmp_path):
        ...
        with patch("initrunner.flow.systemd._UNIT_DIR", unit_dir):
            install_unit("test-proj", flow_path, executable="/usr/bin/initrunner")
    ...
    def test_refuses_without_force(self, mock_check, mock_ctl, tmp_path):
        ...
        with (
            patch("initrunner.flow.systemd._UNIT_DIR", unit_dir),
    ...
    def test_returns_unit_info(self, mock_check, mock_ctl, tmp_path):
        ...
        assert info.flow_name == "test-proj"
        assert info.flow_path == flow_path.resolve()
...
class TestResolveFlowName:
    def test_from_yaml_path(self, tmp_path):
        ...
        assert resolve_flow_name(str(flow_file)) == "my-project"

    def test_from_plain_name(self):
        assert resolve_flow_name("my-project") == "my-project"

    def test_raises_for_nonexistent_yaml(self):
        with pytest.raises(FileNotFoundError, match="not found"):
            resolve_flow_name("/tmp/does-not-exist.yaml")
    ...
    def test_yml_extension(self, tmp_path):
        ...
        assert resolve_flow_name(str(flow_file)) == "yml-project"
