"""Tests for graph-based flow execution."""
...
class TestBuildFlowGraph:
    def test_linear_chain_builds(self):
        ...
        refs = {
            "producer": _make_agent_ref("producer"),
            "consumer": _make_agent_ref("consumer"),
        }
    ...
    def test_fan_out_builds(self):
        ...
        refs = {n: _make_agent_ref(n) for n in ["entry", "svc-a", "svc-b"]}
    ...
    def test_diamond_builds(self):
        ...
        flow = FlowDefinition.model_validate(data)
        refs = {n: _make_agent_ref(n) for n in ["entry", "svc-a", "svc-b", "final"]}
        _graph, entry = build_flow_graph(flow, refs)
        assert entry == "entry"


class TestRunFlowGraph:
    ...
    def test_linear_chain_execution(self, mock_exec):
        ...
        # Build mock FlowMember objects (used by run_flow_graph_sync)
        from initrunner.agent.schema.role import RoleDefinition
    ...
    def test_callbacks_fire(self, mock_exec):
        ...
        _refs, _entry, _elapsed, _timed_out = run_flow_graph_sync(
            flow,
            services,
            "hello",
            entry_service="producer",
    ...
    def test_agent_failure_isolation(self, mock_exec):
        ...
        refs, _entry, _elapsed, _timed_out = run_flow_graph_sync(
            flow,
            services,
            "hello",
            entry_service="entry",
