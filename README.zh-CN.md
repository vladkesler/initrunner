...
> **v2026.4.8**: 成本追踪与 USD 预算管控、成本分析页面、统一底部面板、Flow/Team 工具事件流。查看 [更新日志](CHANGELOG.md)。
...
**授权：** [InitGuard](https://github.com/initrunner/initguard) ABAC 策略引擎（Agent 从角色元数据获取身份，每次工具调用和委托都根据 CEL 策略检查）。参数级权限规则（每个工具的 allow/deny glob 模式，deny 优先）。SQL 授权回调（在引擎层面阻止危险操作）。
