...
### Changed
- **Unknown keys in an agent file are now errors, at every depth.** Only the top level of a flat document, a tool's own keys in a flat document, composed `agents:` children, and the `security` and `observability` sections rejected keys they didn't define; everywhere else a misspelled setting validated clean and was dropped, so `memory: {max_sesions: 3}` ran with the default of 10 and nothing said why. Every model an agent file can reach now uses `extra="forbid"`: model, tools (including `permissions`, script parameters and API endpoints), triggers, sinks, ingest, memory, autonomy, reasoning, guardrails, execution, and the team and flow settings. Envelope files are strict at every level as well, from `metadata` and `spec` down to team personas and flow agents, and so are tool entries and the `requires` block in a skill's frontmatter. Free-form mappings stay open (plugin tool options, MCP `headers` and `env`, `model.extra_headers` and `extra_body`, `output.schema`, `deps_schema`), and a skill's own top-level frontmatter keys are still ignored for agentskills.io compatibility. **A file that loaded before can fail now**; the key it names was never doing anything, so fix the spelling or delete it. The three `memory import` tests that had kept this change out since 2026.8.5 were using `memory: {enabled: true}`, a key `MemoryConfig` never had.

### Fixed
- **Doctor and the starter listing disagreed about which extras a role needs.** They were two scanners over the same YAML. Doctor offered the PDF, Word and Excel parsers (`initrunner[ingest]`) for any `ingest:` block, including a knowledge base of Markdown files that core reads on its own. Both skipped `memory: {}`, which the loader backs with LanceDB, so the `memory` and `telegram` starters didn't list `vector`. Neither noticed `embeddings.provider: local`, which needs `local-embeddings`. Neither looked at inline `agents:` children or shared stores, so the `scholar` starter's `search` child and the `writer` flow's shared memory went unreported. There is one detector now, in the starters service, and doctor filters its answer by what is installed. `ingest` is required only by `.pdf`/`.docx`/`.xlsx` sources (under `ingest` or an enabled `shared_documents`, which reads them through the same parsers) and the `pdf_extract` tool, a section counts whenever it is present (`memory: {}` included), local embeddings are checked under `ingest`, `memory` and `shared_documents`, and enabled `shared_memory`/`shared_documents` need `vector`. Doctor also names the same feature for an extra every time, where before it depended on set iteration order. The starter listing's required-token check reads triggers on inline children too, so a child's `telegram` trigger asks for `TELEGRAM_BOT_TOKEN` along with the `telegram` extra.
- **`initrunner plan` printed `((auto-detect at runtime))` for an agent with no model.** The service already wrapped the label in parentheses and the CLI wrapped it again. It prints one pair now.
- **`initrunner validate` crashed on a team with no `model:`.** The summary table called `to_model_string()` on the missing model and exited with an `AttributeError` traceback, although the file was valid and the team picks a model at run time. The bundled `reviewer` starter is one such team. It now shows `(auto-detect at runtime)`, as it already did for a solo agent.
- **`validate` and the run pre-flight reported a flat file's schema errors as one `document` blob.** Since the flat migration, any Pydantic failure collapsed into a single issue with the real path buried in the message. Each error is its own row again, with its path and fix hint, and every error in the file is listed rather than the first. Tool errors were flattened even earlier, into one "Invalid config for tool" message per list: they now point at the entry and the field (`tools.1.allowd_commands`), and every bad tool is reported, in flat files, envelopes, teams, skills and the dashboard builder alike. Every file a team, flow or group references with `use:` is now checked before the document is composed, so a broken member reports its own fields under its name (`agents.worker.memory.max_sesions`) instead of one message about the file; before, only a bare `use:` in a flow or group got that, and a member with overrides, or any team member, stopped validation at the first bad file. A group or flow whose `use:` led back to itself crashed `validate` with a `RecursionError`; it is an error on that entry now.
- **Saved agent files wrote `output.schema_` instead of `output.schema`.** The flat renderer dumped `OutputConfig` by attribute name, so every file the builder canonicalized or `doctor --fix` migrated got the Python-side key `schema_`, which the loader happened to accept and the docs never mention. It writes `schema` now, and five examples that the 2026.8.4 migration had rewritten are back to the documented key. The editor schema's check against every shipped example caught it.
...
### Changed
- **The Docker images now set `MALLOC_ARENA_MAX=2`**, as do the systemd units `initrunner flow install` writes and the processes `initrunner service start` spawns. Sync and async execution are bridged with worker threads and glibc gives each thread its own malloc arena, which inflates RSS well past the live heap in exactly the containers that are billed on RSS. The docs had recommended this since the memory-footprint page was written; it was set in zero files.
- **Both images seed only `hello-world.yaml` and `code-reviewer.yaml`.** Three of the five seeded examples needed the search, vector or audio extras, which is a broken first impression in an image built without them.

### Added
- **A `slim` Docker image** (`slim`, `<version>-slim`): the core runtime plus the dashboard, 308 MB against 1.07 GB for `latest`, and 127 MB resident against 165 MB serving the same agent. `latest` still carries every extra. Both tags are smoke-tested on publish -- the CLI runs, the allocator cap is set, a seeded example serves and answers `/health`, and every seeded example validates -- and both are scanned by Trivy.
- **`initrunner service status` prints the daemon's RSS** next to its PID, read from `/proc` only once `observe_process()` has verified the PID really is that service. Sizing a fleet starts with knowing what one service costs.
- **A directory of agent files is a group.** `initrunner run agents/ --serve` serves every agent in the directory from one process, `--daemon` runs all their triggers, and `--agent <name>` or `--sense` picks one -- with no group file to write. A directory with no `agent.yaml`/`role.yaml` holding two or more top-level agent documents, and no team, flow or group document beside them, qualifies; members are keyed by the role's own `name:`, which is the model ID a single agent already gets from `--serve`, so moving an agent into a directory does not change how clients address it. Nested directories are ignored (that is where a team's personas live), a team or flow file beside an agent keeps the old "pass one explicitly" error, and one member that fails to load fails the whole thing rather than quietly serving the rest. Write the group file when you need shared memory, shared documents, group-level observability, listener security, or member keys that differ from role names. See [Grouped agents](docs/orchestration/groups.md).
...
### Fixed
- **Anthropic models could not be built at all.** The Anthropic SDK 1.0.0, released 2026-08-20, moved to `httpx2` and type-checks the HTTP client it is handed, so InitRunner's retrying `httpx` client was rejected with a `TypeError` naming `httpx2.AsyncClient` as the only client it accepts. PydanticAI 2.32.1 shipped seventeen hours before that SDK and still builds a legacy `httpx` client for Anthropic itself, so `AnthropicProvider(api_key=...)` raises the same error with no client passed at all; the explicit client InitRunner injects is what makes Anthropic work today. The client is now built in the flavor each SDK accepts: `httpx2` for OpenAI, Anthropic, Google, Mistral, and Cohere, legacy `httpx` for Groq, which is the one SDK that rejects `httpx2`. OpenAI-compatible endpoints (Ollama, vLLM, OpenRouter) follow OpenAI. The floor for `pydantic-ai-slim` moves to 2.32.1, and `httpx2` becomes an explicit core dependency rather than an accident of the resolver. Tracked in [Known gaps](docs/operations/todo.md) with the two upstream changes that will let it be deleted.
- **An install hint reached the user without the extra in it.** A missing extra reports `uv pip install initrunner[vector]`, but Rich reads `[vector]` as markup: on the group `--serve`/`--daemon` path, `initrunner eval`, `role`, `export`, and `doctor`, the user was told to run `uv pip install initrunner`, which is the core package they already have. All of these now escape the message, through one `print_error` helper rather than a repeated local import.
- **The published `latest` tag would have been replaced by the slim build.** `docker/metadata-action` defaults to `latest=auto`, which adds an unsuffixed `latest` on any semver tag, and this workflow only runs on those. The slim build runs second, so it would have overwritten the `[all]` image on both registries while CI still went green. Both metadata steps now set `latest=false` and declare every tag explicitly. While fixing it: `latest` and `slim` are added only for non-pre-release tags, so `v<version>rc1` is a safe full rehearsal of a workflow that otherwise cannot be exercised before a real release; smoke and scan address the immutable version tag rather than a moving one, which would have resolved to the previous release had a push failed; and smoke asserts fastmcp and lancedb import in the full image and are absent from the slim one, the contract that separates them.

### Docs
- [Memory footprint](docs/operations/memory-footprint.md) rewritten from `scripts/measure_rss.py` output: per-layer table with module counts, packed 1/5/50 agents at core and full installs, published image sizes and RSS side by side, the arithmetic for a fleet, and every way to measure it yourself. The old numbers (150-220 MB, +102 MB for LanceDB) predated the dependency split and were never reproducible from a committed script.
- [Docker](docs/getting-started/docker.md) gains a tag table, a "Sizing the container" section with real `docker stats` output, and a Kubernetes CronJob example that calls a warm `--serve` process instead of paying the import cost 1,440 times a day.
...
- **Bumped `h2` to 4.4.1 (CVE-2026-71554, GHSA-6hr6-w5qg-qmwg, medium).** A duplicate `Host` header could facilitate request smuggling. `h2` is a transitive dependency (via `httpx[http2]`, behind the `anthropic` provider) and no first-party module imports it, so this is a lockfile-only bump. It carries `hpack` to 4.2.0.
- **Bumped `dompurify` to 3.4.13 in the dashboard (GHSA-55q2-fjhq-7xh7, medium).** Removing an `IN_PLACE` hook left a detached subtree executable, allowing XSS. It arrives transitively through `posthog-js`, whose range already permitted the patched release, so the lockfile resolve was enough and no `pnpm.overrides` pin was needed.
