...
InitRunner can send anonymous, opt-in product-usage telemetry (command name, outcome, OS, and version, tied to a random id) so the maintainers can see whether and how it is used. It is off until you accept the one-time first-run prompt (CLI) or consent banner (dashboard), and it never sends prompts, file contents, paths, arguments, or API keys. It is distinct from the local audit trail (never transmitted) and from agent observability. Manage it with `initrunner telemetry enable`/`disable`, `DO_NOT_TRACK=1`, or `INITRUNNER_TELEMETRY=off`. Full disclosure of every field is in [`docs/operations/telemetry.md`](../operations/telemetry.md).
