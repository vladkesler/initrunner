...
This creates a directory with `flow.yaml` and `roles/*.yaml` files, all pre-validated. To see the available patterns, run `initrunner flow new my-pipeline --list-patterns`. The NAME argument is required by the command parser, but with `--list-patterns` the command prints the pattern table and exits without creating anything.
...
│        │                     │ (loop-back: writer x4)     │        │         │
│ critic │ ./roles/critic.yaml │ (none)                     │ writer │ none    │
└────────┴─────────────────────┴────────────────────────────┴────────┴─────────┘
Valid
```
...
Flat `flow.yaml` has no per-agent `restart` or `health_check` block. Both are rejected at load time with `Extra inputs are not permitted`, and there is no periodic health-check loop in the orchestrator.

What is tracked is per-agent run and error counts, available via `agent_health()`. In daemon mode, each trigger event spawns an independent graph run; failed runs increment that agent's error counter.

Restart-on-failure is handled by systemd. The unit generated by `initrunner flow install` sets `Restart=on-failure` and `RestartSec=10`. See [Systemd Deployment](#systemd-deployment).
